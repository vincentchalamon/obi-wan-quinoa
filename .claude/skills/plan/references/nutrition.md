...
## 2. Cible énergétique (estimation, à valider par un pro)

Calcul transparent (formule **Mifflin-St Jeor**, 1990) :
- Métabolisme de base = 10×82 + 6,25×178 − 5×39 + 5 = **~1742 kcal/j**.
- Facteur d'activité « légèrement actif » ~1,375 → **maintenance ≈ 2350-2450 kcal/j**.
- Déficit ~500 kcal/j → **cible ≈ 1900 kcal/j**, soit ~0,5 kg/semaine (repère standard de perte de poids progressive). 82→76 kg ≈ 6 kg ≈ ~12 semaines.
- **Auto-régulation** : si le poids ne baisse pas après 2-3 semaines, réduire de ~150-200 kcal/j (ou inversement si perte trop rapide / fatigue).

Les recettes existantes (~900-1000 kcal/repas, ~1800-2000 kcal/j sur 2 repas) sont déjà dans cette zone de déficit léger.
...
L'ANSES a publié le 13/03/2025 ses **premiers repères alimentaires pour les adultes végétariens** (deux expertises : effets santé + repères de consommation). Points retenus pour un lacto-ovo :
- **Difficiles à couvrir : oméga-3 EPA/DHA et vitamine D.** → privilégier les sources d'ALA (huile de colza/noix/lin, noix, graines de lin/chia) ; exposition solaire / vitamine D à surveiller (supplémentation possible sur avis médical).
- **Vitamine B12 à surveiller** même en lacto-ovo : l'ANSES note la très faible biodisponibilité de la B12 des œufs (risque de surévaluation des apports). Les produits laitiers et œufs restent les sources principales ; surveiller si leur consommation est faible.
- **Risque osseux** : l'ANSES associe les régimes végétariens à un risque accru de fractures → soigner **calcium** (laitages) + **vitamine D** + **protéines**.
- **Bénéfices** (niveau de preuve modéré à faible) : risque réduit de diabète de type 2, de cardiopathies ischémiques, de certains cancers.
- **Limite** : ces repères valent pour des adultes en bonne santé ; **les sportifs sont explicitement hors champ**.
...
- Référence : NIH Office of Dietary Supplements, *Iron — Health Professional Fact Sheet*.
...
- **Entrée optionnelle** (crudités, soupe selon saison) quand pertinent.
- **Collation** quand utile, surtout autour du yoga (récupération : protéines + glucides) :
...
- **Printemps** : asperge, artichaut, radis, petit pois, épinard, blette, carotte, navet, oignon nouveau, salade, courgette (fin) ; fraise, rhubarbe, pomme, agrumes (fin).
- **Été** : tomate, courgette, aubergine, poivron, concombre, haricot vert, fève, petit pois, blette, betterave, salade, ail, oignon ; abricot, pêche, cerise, melon, pastèque, framboise, fraise, figue (fin).
- **Automne** : courge/potimarron, poireau, brocoli, chou-fleur, épinard, betterave, carotte, panais, patate douce, champignon, endive ; raisin, pomme, poire, coing, figue, noix, kiwi (fin).
...
- **Mifflin MD, St Jeor ST, et al.** — *A new predictive equation for resting energy expenditure in healthy individuals*, Am J Clin Nutr 1990 (formule de dépense énergétique de repos).
- **ADEME** — calendrier des fruits et légumes de saison.

> Les URL doivent être revérifiées périodiquement (les sites institutionnels réorganisent leurs pages). En cas de doute sur un chiffre, reconsulter la source primaire avant de l'affirmer dans une proposition.
