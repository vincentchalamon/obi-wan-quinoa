...
Demande cible : `$ARGUMENTS` (idée de plat, contrainte, ou ingrédients à valoriser). Si absent, demande-la.
...
## Étapes
1. **Cadrer** : proposer un titre et un concept (saison, panier, objectif protéines) en 2-3 lignes. Itérer avec l'auteur si besoin.
2. **Composer** : ingrédients (quantités réalistes pour 1 personne) et étapes claires.
3. **Chiffrer** : estimer `kcal` et `protéines` (méthode/source), à renseigner dans les champs nutrition de RecipeSage.
4. **Étiqueter** : un label de **type** — `repas` (seule catégorie générée en menu), `base`, `accompagnement` ou `dessert` — et le(s) label(s) de **régime** (additifs) : `vegetarien` (+ `vegan` si aucun produit animal ; un vegan porte aussi `vegetarien`), plus `sans-gluten`/`sans-lactose` si applicable. **Pas de label par ingrédient** ni `midi`/`soir` (inutiles au générateur).
5. **Publier** : présenter le bloc ci-dessous pour validation, puis publier via le script (après accord explicite de l'auteur — action externe) :
   ```sh
   echo '<recette JSON>' | node scripts/rs_publish.mjs --confirm
   ```
   JSON attendu : `{ "title", "yield", "ingredients": [...], "instructions": [...], "labels": [...], "nutritionCalories", "nutritionProtein", "activeTime"?, "totalTime"?, "source"?, "url"? }`. Sans `--confirm` = aperçu (dry-run). Token : voir l'en-tête de `scripts/rs_publish.mjs` (fichier `.rs_token` gitignoré, généré via `users.login`).
